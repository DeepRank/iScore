...
class ScoreGraph(object):
	...
	def __init__(self,testIDs='testID.lst',trainIDs='trainID.lst',
		         graph_path='graphMAT/',gpu_block=(8,8,1),method='vect'):
		...
		Args:
		    ...
		    graph_path (str, optional): Path to the directory containing the graphs
	...
	def import_from_mat(self):
		...
		self.train_graphs = {}
		train_names = self._get_file_names(self.trainIDs,self.graph_path)

		self.test_graphs = {}
		test_names  = self._get_file_names(self.testIDs,self.graph_path)

		for name in train_names:
			self.train_graphs[name] = Graph(self.graph_path + '/' + name)
			self.max_num_edges_train = np.max([self.max_num_edges_train,self.train_graphs[name].num_edges])

		for name in test_names:
			self.test_graphs[name] = Graph(self.graph_path + '/' + name)
	...
	def _get_file_names(filename,dirname):
		...
		if len(names) != len(required_names):
			Warning('Not all required graph found')
